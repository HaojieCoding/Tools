...
def img_to_sample1(img, bbox,f_list, dis):  
    ...
    for f in f_list:
        ...
        # crop box
        off_x = int((side-bbox[2])/2.)
        off_y = int((side-bbox[3])/2.)
        ...
        crop = np.repeat(crop, 5, 0)
        crop = crop + off
        bbox_sample = np.repeat(bbox.reshape(1,4),5,0)
...
def img_to_sample2(img, bbox, dis):  
    ...
    sigma = ( bbox_temp[2]+bbox_temp[3] )/200.0
    samples_neg = tools.generate_samples(img, bbox_temp, 500, 'whole',
                                         sigma*2, 0,-1,0.001,0, dis)
        ...
        for i in range(len(samples_neg)):
            cv2.rectangle(im_copy,
                          ...
                          (samples_neg[i,0]+samples_neg[i,2],
                           ssamples_neg[i,1]+samples_neg[i,3]),
                          (0,255,0), 1)
        plt.imshow(im_copy)
        plt.show()
